...
def update_user_info(user_name,user_id,password):
    #user_name = user_info['user_name']
    logger.debug("inside user update info function !!!!!!!!!!!!!!!!!!!! ")
    user = current.db(current.db.auth_user.username == user_name).select().first()
    if not user:
        user = current.db.auth_user.insert(username=user_name, password=password,user_id=user_id,registration_id=user_name)
    else:
        current.db(current.db.auth_user.username == user_name).update(password=password,user_id=user_id)
